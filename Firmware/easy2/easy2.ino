...
#define THROTTLE_PIN 28       // Throttle pin
#define THROTTLE_LOW 4      // These LOW and HIGH values are used to scale the ADC reading. More on this below
...
#define LED_PIN 25            // The teensy has a built-in LED on pin 13
#define DEADMAN_PIN 12

#define HALL_OVERSAMPLE 4     // Hall oversampling count. More on this in the getHalls() function

// uint8_t hallToMotor[8] = {255, 5, 1, 0, 3, 4, 2, 255}; // Sequence Koford Clockwise
...
void setup() {                // The setup function is called ONCE on boot-up
  ...
  analogWriteFreq(24000); // Set the PWM frequency. Since all pins are on the same timer, this sets PWM freq for all
  ...
  pinMode(DEADMAN_PIN, INPUT);  // Set deadman button pin as input
  delay(2000);
...
void loop() {                         // The loop function is called repeatedly, once setup() is done
 ...
 uint8_t throttle = 0;  // For testing purposes only
  //Serial.println(analogRead(26));
// uint8_t throttle = readThrottle();
  // Check if deadman button is pressed (button open = HIGH, button closed = LOW)
 // if (digitalRead(DEADMAN_PIN) == HIGH) {
   ...
   // Serial.println(throttle);
    //Serial.println(digitalRead(DEADMAN_PIN));
 ...
 // else {
    // If deadman button is not pressed, stop the motor by setting PWM to zero
   // writePWM(255, 0);
  //}
 // Serial.println(digitalRead(DEADMAN_PIN));
  
  // Hall sensor reading (for testing purposes only)
...
uint8_t readThrottle()
{
  ...
  //int32_t adcI = analogRead(26); // Note, analogRead can be slow!
